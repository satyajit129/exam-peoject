...
@section('backend_content')
    <div class="card card-default mb-4">
        <div class="card-header">
            ...
            <a href="{{ route('jobNatureCreateOrEdit') }}" class="btn btn-outline-primary btn-sm">Add New Job Nature</a>
        </div>
        <div class="card-body">
            <table class="table table-bordered">
                ...
                <tbody>
                    @forelse ($job_natures as $job_nature)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $job_nature->name }}</td>
                            <td>
                                @if ($job_nature->employees_count > 0)
                                    <span class="employee-count-badge employees" data-jobnature-id="{{ $job_nature->id }}"
                                        style="cursor: pointer;">{{ $job_nature->employees_count }}</span>
                                @endif
                            </td>
                            <td>
                                <a class="btn btn-sm btn-outline-warning ml-2 " title="Edit"
                                    href="{{ route('jobNatureCreateOrEdit', $job_nature->id) }}">
                                    <i class="mdi mdi-pencil"></i>
                                </a>
                                <a href="javascript:void(0);"
                                    data-url="{{ route('jobNatureDelete', ['id' => $job_nature->id]) }}"
                    ...
                    @empty
                        <tr>
                            <td class="text-center" colspan="3">No data Found! PLease add data from <a
                                    href="{{ route('jobNatureCreateOrEdit') }}">here</a> </td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <script>
        $(document).ready(function() {
            $('.employees').on('click', function() {
                var jobNatureId = $(this).data('jobnature-id');
                $.ajax({
                    url: "{{ route('jobNatureEmployees') }}",
                    type: 'GET',
                    data: {
