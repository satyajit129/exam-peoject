...
@section('title', 'Role Access ' . ($role->id ? 'Update' : 'Create'))
@section('heading', 'Role Access ' . ($role->id ? 'Update' : 'Create'))
...
@section('backend_content')
    <div class="card card-default mb-4">
        <div class="card-header">
            <h2>Role Access {{ $role->id ? 'Update' : 'Create' }}</h2>
            <a href="{{ route('roleAccess') }}" class="btn btn-outline-primary btn-pill btn-sm">Back To list</a>
        </div>
        <div class="card-body">
            <form action="{{ route('roleAccessSave', $role->id ?? '') }}" method="POST">
                @csrf
                <div class="mb-3">
                ...
                <div class="listbox-wrapper">
                    ...
                    <div class="listbox-buttons">
                        <button type="button" id="addPermission" class="btn btn-outline-primary btn-sm">&gt;&gt;</button>
                        <button type="button" id="removePermission" class="btn btn-outline-danger btn-sm">&lt;&lt;</button>
                    </div>

                    {{-- Assigned Permissions --}}
                    <select multiple id="assignedPermissions" name="permissions[]" class="form-control multiple-permission">
                        @foreach ($permissions as $permission)
                            @if (isset($assigned_permissions) && in_array($permission->id, $assigned_permissions))
                                <option value="{{ $permission->id }}">{{ $permission->bangla_code }}</option>
                            @endif
                ...
                <div class="float-end mt-4">
                    <button type="submit" class="btn btn-outline-primary btn-sm">Save Permissions</button>
                </div>
            </form>
        </div>
    </div>
